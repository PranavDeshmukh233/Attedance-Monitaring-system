...
export const registerController = async (req, res) => {
  try {
    const { name, email, password, phone, college, answer } = req.fields;
...
export const forgotPasswordController = async (req, res) => {
  try {
    ...
    //check
    const user = await studentModel.findOne({ email, answer });
    //validation
    if (!user) {
      return res.status(404).send({
        success: false,
    ...
    await studentModel.findByIdAndUpdate(user._id, { password: hashed });
    res.status(200).send({
      success: true,
...
export const updateProfileController = async (req, res) => {
  try {
    ...
    const user = await studentModel.findById(req.user.name);
    //password
